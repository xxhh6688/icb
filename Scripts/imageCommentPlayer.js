let Player = function () {
    ...
    this.showCurrentComment = (realtime) => {
        ...
        this.showCurrentCommentTimer = setTimeout(function () {
            ...
            $.each(self.currentShowComment, function (a, b) {
                ...
                let obj = self.addText(self.ctx, b.text, b.x * $("#myCanvas").width(), b.y * $("#myCanvas").height());
                if (obj) {
                    obj.sprite.id = b.id;
                }
            });
    ...
    this.showImageComment = (start) => {
        ...
        if (!self.needRepaint && (self.currentShowComment.length == 0 || self.imageCommentList[start].id != self.currentShowComment[0].id)) {
            console.log(`need repaint at ${start}`);
            self.needRepaint = true;
        }
    ...
    this.resizeCanvas = function () {
        ...
        $("#image").css("transform", "translate3d({0}px, {1}px, 0px) scale3d({2}, {3}, 1)".format(0, 0, 1, 1));
        $("#image-container").height($("#image").height());
        if ($("#image").height() > $("#main-box").height()) {
            $("#main-box").css("align-items", "unset");
        }
        else {
            $("#main-box").css("align-items", "center");
        }

        $("#myCanvas").css("left", $("#image")[0].getBoundingClientRect().left + "px");
        $("#myCanvas").css("top", $("#image")[0].getBoundingClientRect().top + "px");
        $("#myCanvas")[0].width = $("#image")[0].getBoundingClientRect().width;
        $("#myCanvas")[0].height = $("#image")[0].getBoundingClientRect().height;
    ...
    this.registerEvent = function () {
        ...
        let mouseDown = false;
        $("#myCanvas").unbind("mouseup touchend");
        $("#myCanvas").unbind("mousewheel");
        $("#myCanvas").unbind("mousedown touchstart");
        $("#myCanvas").unbind("mousemove touchmove");
        let oldTime = 0;
        let doubleTouchTime = 0;
        let singleTouchTimer = null;
        $("#myCanvas").bind("mouseup touchend", (e) => {
            if (e.touches) {
                e.preventDefault();
            }
            ...
            singleTouchTimer = setTimeout(function () {
                console.log(`canvas changed: ${self.canvasChanged}`);
                if (self.canvasChanged) {
        ...
        $("#myCanvas").bind("mousewheel", function (e) {
            ...
            self.resizePosition.cx = Math.floor($("#image")[0].getBoundingClientRect().left + $("#image")[0].getBoundingClientRect().width / 2);
            self.resizePosition.cy = Math.floor($("#image")[0].getBoundingClientRect().top + $("#image")[0].getBoundingClientRect().height / 2);
            ...
            $("#image").css("transform", "translate3d({0}px, {1}px, 0px) scale3d({2}, {3}, 1)".format(x, y, scalex, scalex));
            $("#myCanvas").css("left", $("#image")[0].getBoundingClientRect().left + "px");
            $("#myCanvas").css("top", $("#image")[0].getBoundingClientRect().top + "px");
            $("#myCanvas")[0].width = $("#image")[0].getBoundingClientRect().width;
            $("#myCanvas")[0].height = $("#image")[0].getBoundingClientRect().height;
        ...
        $("#myCanvas").bind("mousemove touchmove", function (e) {
            ...
            if (!resizeStart && e.touches && e.touches.length == 1) {
                ...
                $("#image").css("transform", "translate3d({0}px, {1}px, 0px) scale3d({2}, {3}, 1)".format(x, y, self.resizePosition.newScalex, self.resizePosition.newScalex));
                $("#myCanvas").css("left", $("#image")[0].getBoundingClientRect().left + "px");
                $("#myCanvas").css("top", $("#image")[0].getBoundingClientRect().top + "px");
                $("#myCanvas")[0].width = $("#image")[0].getBoundingClientRect().width;
                $("#myCanvas")[0].height = $("#image")[0].getBoundingClientRect().height;
        ...
        methods: {
            init: function () {
                ...
                self.player.clickEvent = (e) => {
                    ...
                    if (!e.touches) {
                        x = e.offsetX / $("#myCanvas").width();
                        y = e.offsetY / $("#myCanvas").height();
                    }
                    else {
                        x = (self.player.dragPosition.x - Math.floor($("#image")[0].getBoundingClientRect().left)) / $("#myCanvas").width();
                        y = (self.player.dragPosition.y - Math.floor($("#image")[0].getBoundingClientRect().top)) / $("#myCanvas").height();
                    }
                    self.startComment();
                    self.position.x = x;
                    self.position.y = y;
